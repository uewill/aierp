...
import 'package:aierp_mobile/features/basic/customer/pages/customer_edit_page.dart';
...
class _CustomerListPageState extends State<CustomerListPage> {
  ...
  Future<void> _loadCustomers({String? keyword}) async {
    ...
    try {
      _customers = await _partnerApi.getCustomerList(keyword: keyword);
    } catch (e) {
      TDToast.showFail('加载客户列表失败', context: context);
    }
  ...
  Widget _buildSearchBar(BuildContext context) {
    return Padding(
      ...
      child: TextField(
        ...
        onChanged: (keyword) {
          _loadCustomers(keyword: keyword.isNotEmpty ? keyword : null);
        },
      ),
    );
  }
  ...
  Widget _buildCustomerList(BuildContext context) {
    ...
    return RefreshIndicator(
      onRefresh: () => _loadCustomers(keyword: _searchController.text.isNotEmpty ? _searchController.text : null),
      child: ListView.builder(
        padding: const EdgeInsets.all(16),
        itemCount: _customers.length,
        itemBuilder: (context, index) => _buildCustomerItem(context, _customers[index]),
      ),
    );
  }
  ...
  Widget _buildCustomerItem(BuildContext context, Map<String, dynamic> customer) {
    ...
    final enabled = customer['enabled'] ?? true;
                    ...
                    Row(
                      children: [
                        Container(
                          ...
                          child: Text(
                            '信用额度: ¥${creditLimit.toStringAsFixed(0)}',
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text('暂无客户数据', style: TextStyle(fontSize: 16, color: Colors.grey[600])),
  ...
  Future<void> _navigateToEdit(Map<String, dynamic>? args) async {
    ...
    if (result == true) {
      _loadCustomers(keyword: _searchController.text.isNotEmpty ? _searchController.text : null);
    }
  }
  ...
  void _showDeleteConfirm(BuildContext context, int? id, String name) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('是否删除客户「$name」？删除后无法恢复。'),
        actions: [
          TextButton(
