...
class SupplierEditPage extends StatefulWidget {
  const SupplierEditPage({super.key});
...
class _SupplierEditPageState extends State<SupplierEditPage> {
  ...
  Future<void> _loadData() async {
    ...
    if (args != null && args is Map<String, dynamic>) {
      _supplierId = args['id'] as int?;
      if (_supplierId != null) {
        setState(() => _isLoading = true);
        try {
          final data = await _partnerApi.getPartner(_supplierId!);
          ...
          TDToast.showFail('加载供应商数据失败', context: context);
        }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: TDNavBar(
        title: _supplierId == null ? '新建供应商' : '编辑供应商',
  ...
  Future<void> _saveSupplier() async {
    ...
    if (_nameController.text.isEmpty) {
      TDToast.showFail('请输入供应商名称', context: context);
      return;
    }
    ...
    try {
      ...
      if (_supplierId != null) {
        await _partnerApi.updatePartner(_supplierId!, data);
